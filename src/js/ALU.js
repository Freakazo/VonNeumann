...
function ALU() {
    ...
    //operation stores the function the ALU should execute.
    this.operation = "0";
...
ALU.prototype.step = function (edge) {
    'use strict';
    if (edge === 0) {
        switch (this.operation) {
            ...
            case "ADC": //Add with carry
                var result = add8Bit(this.A.value, sim.SB.value + this.SREG.C);
                this.C.value = result.value;
                this.SREG.C = result.Cout;
                this.setSREGFlags(this.A.value, sim.SB.value, this.C.value);
                playBusAnim('ALU', 1);
                playALUToAAnim();
                playCoutAnim();
                break;
            case "ADD":
                var result = add8Bit(this.A.value, sim.SB.value);
                this.C.value = result.value;
                this.SREG.C = result.Cout;
                this.setSREGFlags(this.A.value, sim.SB.value, this.C.value);
                playBusAnim('ALU', 1);
...
ALU.prototype.setSREGFlags = function (Rd, Rr, R) {
    'use strict';
...
function DisplayALU(display, ALU) {
    'use strict';
