...
function MEM() {
    ...
    this.MemoryAddress.step = function (edge) {
    ...
    this.MemoryData.step = function (edge) {
        if (this.out === 1 && this.out === 0) {
            console.log("ERROR: MD can't write and read to system bus at the same time");
        }
        ...
        //Read from the system bus
            ...
            this.value = sim.MEM.data[sim.MEM.MemoryAddress.value];
            sim.SB.write(sim.MEM.data[sim.MEM.MemoryAddress.value]);
            this.out = 0;
        ...
        //Write to the system bus
            ...
            this.value = sim.SB.value;
            sim.MEM.data[sim.MEM.MemoryAddress] = sim.SB.value;
            this.in = 0;
            if (this.updateDataValue !== undefined) {
...
function DisplayMemory(display, x, y, memory) {
    ...
    memory.MemoryData.updateDataValue = (function (dataValue, isWrite) {
        ...
        var endX = this.x + 175 + 15 * (sim.MEM.MemoryAddress.value % 10);
        var endY = this.y - 15 + (parseInt(sim.MEM.MemoryAddress.value / 10)) * 15;
    ...
    var MA = new CreateRegister(display, x + 15, y + 30, "MA");
    this.MAValue = MA.value;
    this.MABBox = MA.BBox;
    display.SB.attachToBus(x, y + 40, "MA");

    var MD = new CreateRegister(display, x + 15, y + 65, "MD");
    ...
    for (i = 0; i < 10; i++) {
        for (var iy = 0; iy < 10; iy++) {
            ...
            memCell.node.onmouseover = function () {
                ...
                memCell.valueDisplay = display.paper.text(memCell.node.memX + 3, memCell.node.memY + 3, display.SIM.MEM.data[index]);
